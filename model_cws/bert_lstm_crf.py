...
class BERT_LSTM_CRF(nn.Module):
    ...
    def get_output_score(self, sentence, attention_mask=None):
        ...
        outputs = self.word_embeds(sentence, attention_mask=attention_mask)
    ...
    def forward(self, sentence, masks):
        lstm_feats = self.get_output_score(sentence, attention_mask=masks)
        scores, tag_seq = self.crf._viterbi_decode(lstm_feats, masks.bool())
        return tag_seq
